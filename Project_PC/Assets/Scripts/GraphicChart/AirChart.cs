...
public class AirChart : MonoBehaviour {
    ReceiveData receiveData = new ReceiveData();
    WMG_List<string> xAxisLabels = new WMG_List<string>();//设置横轴名    
    WMG_List<Vector2> pointvalues1 = new WMG_List<Vector2>();    //柱形图的设置  
    WMG_List<Vector2> pointbalues2 = new WMG_List<Vector2>();   //折线图点的设置
    WMG_Axis_Graph barGraph;
    WMG_Series series1;    //柱形图
   // WMG_Series series2;    //折线图

    Text todayUseAmount;  //今日用电量
    Text nowTemperature;  //当前温度
    Text nowPower;      //当前功率
    Text setTemperature;  //设定温度
    Text nowState;      //当前设定
    ...
    void Awake()
    {
        ...
        {
            int a = int.Parse(receiveData.airData[i]);
            totalTime.Add(a);
        }
        Hello();
        GetData();
        dataLabelsParent = transform.Find("LineGraphAir/Series/Series1/dataLabelsParent"); //一定要放在最后
    ...
    {   //柱形图与x轴有缝隙
        ...
        RectTransform line;
        line = transform.Find("LineGraphAir/Background/XAxis/Line").GetComponent<RectTransform>();
