...
public class PhotovoltaicChart : MonoBehaviour
{
    ...
    void Awake()
    {
        receiveData.PhotovoltaicCar("pvCar");
        for (int i = 27; i <= 99; i += 4)
        {
            int a = int.Parse(receiveData.photovoltaicCar[i]);
            totalTime.Add(a);
        }
        Hello();
        GetData();
        dataLabelsParent = transform.Find("BarGraphChe/Series/Series1/dataLabelsParent"); //一定要放在最后
    ...
    private void GetData()
    {
        ...
        todayGeneEnergy.text = float.Parse(receiveData.photovoltaicCar[2]).ToString("0.0") + "kWh";
        nowPower.text = float.Parse(receiveData.photovoltaicCar[4]).ToString("0.0") + "kW";
        todayEqualHour.text = float.Parse(receiveData.photovoltaicCar[6]).ToString("0.0") + "h";
        todayGenePowerProfit.text = float.Parse(receiveData.photovoltaicCar[8]).ToString("0.0") + "元";
        float max = 0;
        for (int i = 27; i <= 99; i += 4)
        {
            xAxisLabels.Add(receiveData.photovoltaicCar[i]);    //添加x坐标

            pointvalues1.Add(new Vector2(float.Parse(receiveData.photovoltaicCar[i]), float.Parse(float.Parse(receiveData.photovoltaicCar[i + 2]).ToString("0"))));

            float num = float.Parse(float.Parse(receiveData.photovoltaicCar[i + 2]).ToString("0"));
    ...
    {   //柱形图与x轴有缝隙
        ...
        RectTransform line;
        line = transform.Find("BarGraphChe/Background/XAxis/Line").GetComponent<RectTransform>();
