...
public class PhotovoltaicRoof : MonoBehaviour {
    ...
    void Awake()
    {
        receiveData.PhotovoltaicRoof("pvRoof");
        for (int i = 27; i <= 99; i += 4)
        {
            int a = int.Parse(receiveData.photovoltaicRoof[i]);
            totalTime.Add(a);
        }
        Hello();
        GetData();
        dataLabelsParent = transform.Find("BarGraphRoof/Series/Series1/dataLabelsParent"); //一定要放在最后
    ...
    void Hello()
    {
        ...
        barGraph = GameObject.Find("BarGraphRoof").GetComponent<WMG_Axis_Graph>();
        //删除自带的两个series
        ...
        //y轴设置
      //  barGraph.yAxis.AxisMaxValue = 50;
    ...
    private void GetData()
    {
        ...
        todayGeneEnergy.text = float.Parse(receiveData.photovoltaicRoof[2]).ToString("0.0") + "kWh";
        nowPower.text = float.Parse(receiveData.photovoltaicRoof[4]).ToString("0.0") + "kW";
        todayEqualHour.text = float.Parse(receiveData.photovoltaicRoof[6]).ToString("0.0") + "h";
        todayGenePowerProfit.text = float.Parse(receiveData.photovoltaicRoof[8]).ToString("0.0") + "元";

        float max = 0;
        for (int i = 27; i <= 99; i += 4)
        {
            xAxisLabels.Add(receiveData.photovoltaicRoof[i]);    //添加x坐标

            pointvalues1.Add(new Vector2(float.Parse(receiveData.photovoltaicRoof[i]), float.Parse(float.Parse(receiveData.photovoltaicRoof[i + 2]).ToString("0"))));

            float num = float.Parse(float.Parse(receiveData.photovoltaicRoof[i + 2]).ToString("0"));
    ...
    {   //柱形图与x轴有缝隙
        ...
        RectTransform line;
        line = transform.Find("BarGraphRoof/Background/XAxis/Line").GetComponent<RectTransform>();
